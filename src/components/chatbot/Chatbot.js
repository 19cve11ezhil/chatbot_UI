...
const ChatApp = () => {
  ...
  console.log(api_url,"bot backend")
  ...
  const sendMessage = async () => {
    ...
    const val = messages
    val.push(userMessage)
    setMessages(val);

    try {
      const response = await axios.post(`${api_url}/get/response/from_bot`, { message: inputMessage });
      const botResponse = {
